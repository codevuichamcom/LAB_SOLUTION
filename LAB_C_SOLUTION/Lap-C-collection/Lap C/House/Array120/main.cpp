...
void deleteElement(int *arr, int &size, int pos){
    ...
    for(int i=pos;i<size-1;i++){ //delete location
        arr[i]=arr[i+1];
    }
    size--;
...
void deleteFirst(int *arr, int &size){
    ...
    int temp = -999;
    //printf("temp la %p", &temp);
    for(int i=0;i<size-1;i++){
        for(int j=i+1;j<size;j++){
            if(arr[i]==arr[j] && checkFirstTime == 0 ||
                   arr[i]==arr[j] && arr[i] != temp){ 
                temp = arr[i];
                printf("temp la %d", temp);
                //if(arr[i] == temp) break;
                deleteElement(arr,size,i); //delete first location
                checkFirstTime = -1;
                //printf("\nsize: %d", size);
